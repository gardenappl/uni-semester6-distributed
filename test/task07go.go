...
//Multiple trains can go in one direction in one tunnel at once

func train(
	index int,
	comingFromNorth bool,
	startOnA bool,
	movingInA *int, //<0 if moving south, >0 if moving north
	movingInB *int, //<0 if moving south, >0 if moving north
	...
	for true {
		...
		if waitingOnA && comingFromNorth {
			...
			//sadly Golang does not have an easy way to wait with timeout
			for true {
				success := false
